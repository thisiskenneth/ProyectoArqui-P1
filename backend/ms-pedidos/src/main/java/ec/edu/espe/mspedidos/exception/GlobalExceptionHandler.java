package ec.edu.espe.mspedidos.exception;
...
public class GlobalExceptionHandler {
    ...
    /** UUID o tipo inválido en path/query */
    @ExceptionHandler(MethodArgumentTypeMismatchException.class)
    public ResponseEntity<Map<String, Object>> handleTypeMismatch(MethodArgumentTypeMismatchException ex) {
        String message = String.format("El parámetro '%s' con valor '%s' no es válido. Se esperaba tipo: %s",
                ex.getName(), ex.getValue(), ex.getRequiredType() != null ? ex.getRequiredType().getSimpleName() : "desconocido");
        log.warn("Error de tipo de parámetro: {}", message);
